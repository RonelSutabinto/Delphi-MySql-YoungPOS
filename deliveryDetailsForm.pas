...
begin
  tmpQ.Close;
  tmpQ.SQL.Clear;
  tmpQ.SQL.Add('Select quantity as quantity2, ifnull(quantity,0)- (ifnull( (Select sum(ifnull(quantity,0))       ');
  tmpQ.SQL.Add('                                                       as quantity from deliverydtls ');
  tmpQ.SQL.Add('                                      where iddeliverydtls <> :iddeliverydtls and    ');
  tmpQ.SQL.Add('                                            idjodetails = :idjodetails),0)           ');
  tmpQ.SQL.Add('                            +:qnty)  as quantity                                     ');
  tmpQ.SQL.Add(' from jodetails where idjodetails = :idjodetails                                     ');
  ...
  if ( tmpQ.FieldByName('quantity').AsInteger <0 )then
  begin
    clearDetailsQ.ParamByName('idjodetails').Text := idjodetails_;
    clearDetailsQ.Execute;

    messagedlg('Quantity not more than '+tmpQ.FieldByName('quantity2').text+'..',mterror,[mbOK],0);
    exit;
  end;
