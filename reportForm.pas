...
type
  TReports = class(TForm)
    ...
    delRepQTAmount: TFloatField;
    delRepQcoveredDate: TStringField;
    countDelQ: TMyQuery;
    countDelQcnt: TLargeintField;
...
begin
  ...
  begin
     joQ.Close;
    joQ.FilterSQL := ' Delivered in ( ''No'',''Yes'') ';
    joQ.Filtered  := true;
    joQ.Open;
  end;
end;
...
begin
  ...
  AdvDateTimePicker7.Date := StartOfTheMonth(now);
  AdvDateTimePicker8.Date := EndOfTheMonth(now);
...
begin
  ...
  delRepQ.ParamByName('dateCovered').Text := 'Date Covered: '+FormatDateTime('m/d/YYYY', AdvDateTimePicker7.Date)+'-'+
                                                FormatDateTime('m/d/YYYY', AdvDateTimePicker8.Date);
  delRepQ.Open;

  delReport.ShowReport;
end;
...
begin
  ...
  countDelQ.Close;
  countDelQ.ParamByName('date').AsDate := now;
  countDelQ.Open;

  countLabel.Caption := joDueCountQcnt.Text;
  delCount_l.Caption := countDelQcnt.Text;
end;
