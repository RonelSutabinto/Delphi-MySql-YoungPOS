object searchJO: TsearchJO
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = 'Search Unpaid Job Order'
  ClientHeight = 395
  ClientWidth = 678
  Color = clBtnFace
  ...
  object AdvPanel1: TAdvPanel
    ...
    object AdvSmoothLabel1: TAdvSmoothLabel
      ...
      Fill.ShadowOffset = 0
      Fill.Glow = gmNone
      Caption.Text = '    Search Unpaid Job Orders'
      ...
      CaptionShadow.Text = '    Search Unpaid Job Orders'
  ...
  object joQ: TMyQuery
    ...
    SQL.Strings = (
      ...
      '  sum(ifnull(c.amount,0))  as AmountPaid,'
        ...
        '  sum(ifnull(jd.quantity,0)*ifnull(jd.amount,0)-ifnull(jd.discou' +
        'nt,0)) - sum(ifnull(c.amount,0)) Balance,'
  ...
  object collectionSumQ: TMyQuery
    Connection = DataModl.MyConnection1
    SQL.Strings = (
      
        'Select sum(ifnull(amount,0)) as amount from collection where idJ' +
        'O = :idjo')
    Left = 688
    Top = 40
    ParamData = <
    ...
    object collectionSumQamount: TFloatField
      FieldName = 'amount'
    end
  end
  object jodetailSumQ: TMyQuery
    Connection = DataModl.MyConnection1
    SQL.Strings = (
      
        'Select sum((ifnull(quantity,0)*ifnull(amount,0))) as amount from' +
        ' jodetails where idjo = :idjo')
    Left = 712
    Top = 40
    ParamData = <
      item
    ...
    object jodetailSumQamount: TFloatField
      FieldName = 'amount'
    end
  end
end
